...
namespace RGSK
{
    ...
    {
        public static Skidmark instance;
        ...
        public int AddSkidMark(Vector3 pos, Vector3 normal, float intensity, int lastIndex)
        {
            ...
            if (lastIndex != -1)
            {
                ...
                curSection.Posl = curSection.Pos + 0.5f * MARK_WIDTH * xDir;
                curSection.Posr = curSection.Pos - 0.5f * MARK_WIDTH * xDir;
                curSection.Tangent = new Vector4(xDir.x, xDir.y, xDir.z, 1);

                if (lastSection.LastIndex == -1)
                {
                    lastSection.Tangent = curSection.Tangent;
                    lastSection.Posl = curSection.Pos + 0.5f * MARK_WIDTH * xDir;
                    lastSection.Posr = curSection.Pos - 0.5f * MARK_WIDTH * xDir;
                }
            }
