	...
	public class RayShoot : DamageBase
	{
		...
		void ShootRay ()
		{
			...
			if (Physics.Raycast (this.transform.position, this.transform.forward, out hit, Range)) {
				...
				hit.collider.gameObject.SendMessage ("ApplyDamage", (float) damagePack.Damage, SendMessageOptions.DontRequireReceiver);
				hit.collider.SendMessage("RecieveDamage", damagePack.Damage, SendMessageOptions.DontRequireReceiver);

				//if (hit.collider.GetComponent<AIManager>())
				//{
				//	hit.collider.GetComponent<AIManager>().GetDamage(Damage);
				//}
				//if (hit.collider.tag == "TrafficCar")
				//{
				//	if (hit.collider.GetComponentInChildren<MeshDestroy>() != null)
				//	{
				//		hit.collider.GetComponentInParent<TSSimpleCar>().crashed = true;
				//		hit.collider.GetComponentInChildren<MeshDestroy>().DestroyMesh();
				//	}
