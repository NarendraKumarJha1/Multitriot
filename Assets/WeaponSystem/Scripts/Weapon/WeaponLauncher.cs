...
namespace HWRWeaponSystem
{
    ...
    public class WeaponLauncher : WeaponBase
    {
        ...
        public bool isFireForward = false;
        ...
        private void Start()
        {
            ...
            //    Missile = bulletPrefabs[GameManager.Instance.CurrentPlayer];
        }
        private void OnEnable()
        {
            GameController.updateMainCamera += UpdateMainCamera;
        }
        private void OnDisable()
        {
            GameController.updateMainCamera -= UpdateMainCamera;
        }
        void UpdateMainCamera(Camera updatedCamera)
        {
        ...
        void FixedUpdate()
        {
            ...
            //    isFireForward = true;
            //}
            //if (this.gameObject.tag != "AirTarget")
            //{
            //    if (isFireForward == false)
            //    {
            //        if (Camera.main)
            //        {
            //            Ray ray = Camera.main.ScreenPointToRay(new Vector2(Screen.width / 2, Screen.height / 2));
            //            Vector3 lookAt = ray.direction * 500f + this.transform.position;
            //            lookAt.y += 8f;
            ...
            //{
            //    if (VirtualInput.GetKeyDown("Shootgun"))
            //    {
            //        Shoot();
            //    }
            //}
            ...
            //{
            //    if (VirtualInput.GetKeyDown("Rocket"))
            //    {
            //        Shoot();
            //    }
            //}
        ...
        private void DrawCrosshair()
        {
            ...
            if (CurrentCamera)
            {
                ...
                if (CrosshairTexture)
                {
                    GUI.DrawTexture(new Rect(crosshairPos.x - CrosshairTexture.width / 2, Screen.height - crosshairPos.y - CrosshairTexture.height / 4, CrosshairTexture.width, CrosshairTexture.height), CrosshairTexture);
        ...
        public void Shoot(string carname)
        {
            if (InfinityAmmo)
            {
                Ammo = 1;
                ...
                if (Time.time > nextFireTime + FireRate)
                {
                    ...
                    for (int i = 0; i < NumBullet; i++)
                    {
                        if (Missile)
                        {
                            ...
                            Vector3 direction = this.transform.forward + spread + bulletOffset;
        ...
        public void ShowCrossair(bool status)
        {
            ShowCrosshair = status;
        }
    }
}
