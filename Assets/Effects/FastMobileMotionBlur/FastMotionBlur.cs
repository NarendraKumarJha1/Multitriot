...
public class FastMotionBlur : MonoBehaviour {
	...
	private void Update () {
		if (!playerCar) playerCar = FindObjectOfType<RCC_CarControllerV3> ();
		// Debug.Log (playerCar.speed + "       /       " + playerCar.orgMaxSpeed);
		Distance = (playerCar.speed / playerCar.orgMaxSpeed) * 0.004f;
		// Distance = Mathf.Clamp (, 0, 0.05f);
	}
